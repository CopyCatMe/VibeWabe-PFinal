...
function HeaderMenu({ isOpen, toggleOpen }) {
  return (
    <nav className={`flex items-center justify-between p-1 w-full h-15 m-3 transition-all duration-300 ${isOpen ? '' : 'ml-[-230px]'}`}>
      ...
      <div className="flex justify-center items-center gap-4 bg-[#1c1c1c] w-1/7 p-5 shadow-md rounded-3xl">
        <Bell className="text-gray-400 hover:text-white cursor-pointer" />
        <MessageSquare className="text-gray-400 hover:text-white cursor-pointer" />
        <div className="h-6 w-px bg-gray-600" />
        <User className="text-gray-400 hover:text-white cursor-pointer rounded-full border-2 border-gray-600" />
